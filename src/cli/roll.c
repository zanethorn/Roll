...
void print_usage(FILE *stream, const char *program_name) {
    ...
    fprintf(stream, "    %s '4d6h3'    # Keep highest 3 of 4d6 (h is alias for k)\n", program_name);
    fprintf(stream, "    %s '5d6l2'    # Drop lowest 2 of 5d6\n", program_name);
    fprintf(stream, "    %s '4d6k'     # Keep 1 highest (default)\n", program_name);
    fprintf(stream, "    %s '8d4l'     # Drop 1 lowest (default)\n", program_name);
